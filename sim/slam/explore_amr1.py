...
# 🔑 북쪽 띠를 y=16.3 으로 올렸다 (예전 15.2). 벽쪽 맨 북단 차의 **앞
#    번호판** 관측점이 (3.2, 16.45) 라, 예전 지도(북단 15.41)로는 그 점이
#    지도 밖이라 amr1_nav.goal_blocked() 가 무조건 출동을 막았다.
#    벽 A 는 y=0~14 라 북쪽 띠에는 벽이 없다. 바닥 북단은 17.0.
WAYPOINTS = [
    (0.8, 16.3),      # 서쪽 통로를 끝까지 올라간다
    (3.2, 16.3),      # 벽쪽 맨 북단 차의 앞 번호판 관측점 위 — 여길 밟아야
                      # (3.2, 16.45) 주변이 '확실한 빈 공간' 으로 찍힌다
    (12.7, 16.3),     # 북쪽 띠를 따라 중앙 통로 입구로
    (12.7, 0.0),      # 중앙 통로 종단 — 양쪽 블록이 전부 스캔에 들어온다
    (12.7, -3.5),     # 남쪽 띠로 빠져나온다
    (28.8, -3.5),     # 남쪽 띠를 따라 동쪽 끝으로
    (28.8, 16.3),     # 동쪽 통로를 올라간다 — 소방차 구역이 들어온다
    (14.0, 16.3),     # 북쪽 띠를 되짚어 간다 — 🔑 루프 클로저
]
