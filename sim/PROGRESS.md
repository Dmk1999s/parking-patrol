# 진행 기록 — Isaac Sim 주차 단속 시뮬 (2026-08-18 세션 5 — EV 파란판 수정 + 통짜 리허설 3)
...
1. **외곽 벽 + 지도 재작성 + AMCL** 🚧 (다음 세션 1순위, 사용자 결정)
   지금은 시뮬 odom 이 참값이라 정적 TF `map→odom`(항등)으로 국지화를
   **통째로 우회**하고 있다 — 실물 격차 중 제일 크다. 그런데 🚨 **외곽에
   벽이 아예 없어서**(`GROUND=(-2,30,-6,17)` 바닥판 + 내부 칸막이 2개뿐)
   라이다가 맞힐 특징이 부족해 AMCL 이 발산할 위험이 크다. 순서:
   ① 외곽 담장 추가 (북쪽은 **y=17.0** — 관측점 16.42 + 로봇반경 0.28 =
   16.70 이라 여유 0.3 m. 더 당기면 앞판 관측점이 다시 UNREACHABLE)
   ② SLAM 지도 재작성 (⚠ **스캔 매칭은 계속 끈다** — 시뮬 odom 이 참값이라
   매칭 없이 그린 지도가 기하학적으로 가장 정확하고 AMCL 은 거기에 스캔을
   맞추는 쪽이다. 켜면 지도가 뒤틀린다. 벽이 생기면 "무반사 광선은 free 로도
   안 그려진다" 문제가 사라져 로봇을 북단까지 밀 필요도 없다)
   ③ AMCL 투입 — `nav2_params_*.yaml` 에 `amcl` 섹션(`odom_frame_id:
   amr1/odom`, `base_frame_id: amr1/base_link`, `scan_topic: /amr1/scan`,
   `laser_max_range` 20), 런치에서 **정적 TF 노드 제거** + `amcl` 추가 +
   라이프사이클 `node_names` 에 등록, 초기 위치(amr1 map (0,0) ·
   amr2 map (2.0, 0.0)).
   ⚠ **커닝을 끄는 작업이라 지금의 11/11 정답이 흔들릴 수 있다** — 세션 5
   리허설 결과가 비교 기준선이다.
2. **depth 차폭 — 진짜 경차 미검증** 세션 5 에서 1.82 · 1.78 m (실제 1.80)
   로 안정적이었다 (세션 4 의 1.74/1.81/**2.64** 같은 튐 없음). 하지만
