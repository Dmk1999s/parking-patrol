...
## 세션 7 (2026-08-19) — 고시 별표 규격 재현 + OCR EasyOCR 전환 ✅

### 배경 — "실제 차량 번호판과 동일하게 만들 수 없나"

세션 6 까지 번호판은 **나눔고딕Bold 로 대충 그린 그림**이었다. 실물에 맞추려
하자 자료가 세 단계로 들어왔고, 단계마다 앞 단계의 결론이 뒤집혔다.

| 자료 | 뒤집힌 것 |
|---|---|
| (없음) 추정 | 글자 높이 64.4 mm 로 그려지고 있었다 (거리표는 80 전제) |
| 보도자료 (2018-12-21) | "신형은 전용 서체" 가설 ❌ — **현행 서체 유지**가 원문 |
| **고시 별표 원문** | 80 mm ❌ → **74.8**, 띠 55 ❌ → **65**, "한글 뒤 공백" ❌ → **없음** |

교훈: 2차 자료로는 방향만 맞고 값은 틀린다. §9 의 별표가 오기 전까지 잰
값은 전부 근사였다.

### 규격 — 별표 2-2 (비사업용 8자리 필름부착방식 520×110)

```
띠 65.0 │ 숫자 50.0 ×3 │ 한글 85.0 │ 숫자 50.0 ×4 │ 우여백 20.0   = 520
```

- 🚨 **셀 높이 85.0 과 잉크 높이 74.8 은 다르다.** 셀로 읽으면 14% 크게 그린다
- 🚨 **한글 뒤 공백은 없다.** 한글 셀이 85 mm 로 넓어 여백이 남는 것이었다.
  보도자료 사진만 보고 `GAP_W` 로 구현했다가 도면으로 바로잡았다
- 번호 8자리 전환 시 `rng` 소비량이 달라질 수 있다 — `plan()` 을 통째로
  비교해 seed 7 배치(차종·색·판정)가 그대로임을 확인하고 넘어갔다
- EV 판은 [별표 18] 대로 **연한 하늘색(PANTONE 290C) + 검은 글자**.
  진한 남색 + 흰 글자로 그리던 게 실물과 **정반대**였다

### 서체 — 폰트 파일 없이 도면을 오려 썼다

별표 도면이 **JPEG 래스터**라 TTF 로 만들면 재현본이 되지만, 오려 쓰면 원본이다.
`scenes/plate_glyphs.py` 가 45자(숫자 10 + 한글 35)를 뽑는다.

가르는 열쇠는 **획 두께**다 (획 ~55 px vs 치수선 1~3 px, `MORPH_OPEN(15×15)`).
🚨 **연결요소 넓이로 거르면 안 된다** — 치수선이 글자에 닿아 있어 같은 요소가
되고 bbox 가 페이지 전체(263×**1912**)로 늘어난다. 두 번 밟았다.

덤: `layout._HANGUL` 에 **실제 번호판에 없는** 차·카·타·파 와 사업용
바·사·아·자, 택배 배 가 섞여 있었다. 별표의 35자로 교체.

### OCR — tesseract 가 무너져 EasyOCR 2단계로

실제 서체를 넣자 §0 의 예고가 그대로 실현됐다.

| 표본 70칸 (2 m, σ0.9) | 결과 |
|---|---|
| tesseract | 43/70 |
| EasyOCR 통짜 | 60/70 |
| **EasyOCR + 한글 칸 재독** | **70/70** |

문자 단위로 가르니 **숫자 490/490(100%), 한글 60/70** 이었다 — 틀리는 건
넷째 자리 하나뿐. 🚨 **신뢰도로 못 거른다** (오답 최대 1.00, 정답 최소 0.45).

셀 배분을 갖고 있으므로 **텍스트 상자를 비율로 갈라** 한글 칸만 한글 전용
allowlist 로 재독하면 끝났다. 판 사각형 정밀 검출은 필요 없었다.
장당 35 ms (tesseract 는 subprocess 6회라 487 ms).

### 🚨 배선 — venv 두 개를 섞는 함정

easyocr 은 `~/ocr_venv` 에 격리 설치했다 (`isaacsim_venv` 의 torch 2.11 을
건드리면 Isaac 이 깨진다). ROS 패키지는 PYTHONPATH 로 얹으면 한 프로세스에서
rclpy + easyocr 이 같이 돈다. 다만 핀 두 개를 **같은 명령으로** 걸어야 한다:

| 패키지 | 핀 | 어기면 |
|---|---|---|
| `numpy` | <2 | `cv_bridge` 가 numpy 1.x C 확장 → `_ARRAY_API not found` |
| `opencv-python-headless` | <5 | Humble `cv_bridge` 타입 상수 불일치 → `KeyError: 16` |

따로 걸면 뒤엣것이 numpy 2 를 도로 끌어온다 (실제로 되돌아갔다).

### 🚨 결함 — EV 판별. 통짜 리허설이 잡았다

첫 회차 **10/11**. EV 구역 은색 세단이 `plate_blue_run=273` 으로 EV 오판돼
CLEARED 로 빠졌다 (진짜 EV 는 243 — **오탐이 더 컸다**).

`PATROL_ZONE_DEBUG` 로 판정 시점 프레임을 덤프해 재니 가설이 틀렸다:

| | 채도 S |
|---|---|
| EV 판 바탕 | **32** (텍스처 54 → 렌더에서 떨어짐, V=218) |
| 흰 판 바탕 | 1 |
| 차체 | 7~16 |

**문턱 35 가 EV 판(32)보다 높았다.** 진짜 판이 통째로 안 잡히고 채도 높은
가장자리만 열당 ~16행 얇게 걸렸는데, 그게 흰 판 가장자리 잡음(열당 ~4행)과
구별이 안 돼 청색 띠에서 시작한 run 이 차체를 타고 이어붙었다.
...
2차원 스윕으로 안전 영역이 채도 22~30 × 열두께 8~20 임을 확인하고 그 한가운데를
골랐다. 수정 후 같은 프레임에서 **EV 179 vs 세단 37** — 4.8배.
...
녹화 `rehearsal5_20260819.mp4` (89분 → 30배속 3분, 오버헤드 + AMR1/2 OcrCam).
서버가 내주는 프레임을 받아 합치는 방식이라 씬에 손대지 않았다.

### 🚨 새로 드러난 것 — 경차 차폭 마진이 3.3 mm

| stall 5 세단 (실제 1.8 m) | 값 |
|---|---|
| 세션 5·6 | 1.77 · 1.82 · 1.80 · 1.82 |
| 오늘 1회차 | 1.8017 |
| **오늘 5회차** | **1.7033** ← 임계 1.70 과 **3.3 mm** |

`PROGRESS.md` 의 "마진 0.08" 서술이 틀렸다. 게다가 방향이 위험한 쪽이다 —
**과소 측정되면 세단이 경차로 넘어가 단속을 놓친다.** 오늘 stall 3 은 1.93,
1회차엔 2.31 로, 같은 차가 0.6 m 폭으로 흔들렸다.

원인 후보는 **도착 거리**다. `car_width_depth` 는 관측점이 판에서 2 m 라고
전제하고 픽셀→미터를 환산하는데, EV 프레임에서 판 폭이 292 px(기대 241)로
나온 걸 보면 실제로는 더 가까이 선다. depth 로 **실제 거리를 읽어 환산**하면
자세 흔들림이 상쇄될 것이다. 미해결.
