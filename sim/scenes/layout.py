...
## 차량 종류를 AMR 이 어떻게 아는가 (사용자 확정)
    ...
    경차     차체가 작다 (1.60×3.60 vs 일반 1.80×4.70) — 크기로 판별
...
# 주차장 바닥 범위 (x0, x1, y0, y1) — 이 밖은 웹캠 프레임에서 어두운 띠다.
# 🚨 북단이 17.0 → 17.4 로 늘었다 (세션 6, 외곽 담장). 담장을 바닥 **안쪽**에
#    세우면 안쪽 면이 16.6 이 되어 앞판 관측점(16.42 + 로봇반경 0.28 = 16.70)
#    이 벽에 먹힌다 — 살려 둔 앞판 단속이 다시 UNREACHABLE 이 된다.
#    바닥을 담장 두께만큼 넓혀 안쪽 면을 정확히 17.0 에 둔다 (여유 0.3 m).
GROUND = (-2.0, 30.0, -6.0, 17.4)

# ── 외곽 담장 ───────────────────────────────────────────────────
# 라이다가 맞힐 게 내부 칸막이 2개와 차들뿐이라 AMCL 이 발산할 위험이 컸다.
# 사방을 두르면 ① 국지화 특징이 생기고 ② "무반사 광선은 free 로도 안 그려진다"
# 문제가 사라져 지도 북단을 만들려고 로봇을 끝까지 밀 필요가 없다 (세션 4 에서
# 그러다 차에 박았다). 실제 주차장이 사방 막혀 있으니 충실도도 올라간다.
# 담장은 바닥 경계 **안쪽**에 두께만큼 들어와 선다 (바닥 위에 서야 한다).
FENCE_T = 0.4              # 담장 두께
FENCE_H = 1.5              # 담장 높이 — 내부 칸막이(1.2)보다 높게, 차(1.45) 위로

# 오버헤드 웹캠 — 형상은 lot.py 가 세우지만 숫자는 여기 둔다.
# sim/vision/webcam_detect.py 가 픽셀→월드 역변환에 같은 값을 써야 해서다
# (그쪽은 Isaac(pxr) 을 import 할 수 없다). 렌더는 1280×720.
# 카메라는 무회전 수직 하방(-Z) → 이미지 +u = 월드 +X, 이미지 +v = 월드 -Y.
...
# ── 차량 ────────────────────────────────────────────────────────
#   body   = (폭, 길이, 높이)   cabin = (폭, 길이, 높이)  — 총고 = body+cabin
#   ev     = 파란 번호판을 쓰는가
#
# 실제 규격에 맞춤 (2026-08-17, 사용자 확정 "차량만 실측 치수로"):
#   SEDAN/EV  준중형~중형 승용차: 1.80 × 4.70 × 총고 1.45 (아반떼~쏘나타급)
#   COMPACT   경차 규격 한계 1.6×3.6, 모닝 실측 1.595×3.595×1.485
#   FIRE      중형 펌프차: ~2.5 × 7.5 × 총고 3.2 (⚠ FIRE_AREA 깊이 8.0 안에
#             들어가야 해서 길이는 7.5 로 잡는다 — 실차 7~9 m 범위)
# ⚠ 치수를 바꾸면 SLAM 지도(차가 장애물로 찍힘)·웹캠 탐지 밴드·observation
#   좌표(번호판 위치)가 전부 갈린다 — 지도 재작성 + 탐지 재검증 필수.
VEHICLES = {
    "SEDAN":   dict(body=(1.80, 4.70, 0.85), cabin=(1.62, 2.30, 0.60), ev=False),
    "COMPACT": dict(body=(1.60, 3.60, 0.80), cabin=(1.44, 1.80, 0.65), ev=False),
    "EV":      dict(body=(1.80, 4.70, 0.85), cabin=(1.62, 2.30, 0.60), ev=True),
    "FIRE":    dict(body=(2.50, 7.50, 2.00), cabin=(2.30, 2.60, 1.20), ev=False),
}
